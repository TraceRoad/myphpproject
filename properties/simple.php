...
*/
class mysql{
    private $host    ='localhost:3306'; //数据库主机
    private $user     = 'root'; //数据库用户名
    private $pwd     = 'root'; //数据库用户名密码
    private $database = 'library'; //数据库名
    ...
    private function __construct($pconnect = false) {
        if (!$pconnect) {
            $this->link = mysqli_connect($this->host, $this->user, $this->pwd) or $this->err();
        } else {
            $this->link = mysqli_connect($this->host, $this->user, $this->pwd) or $this->err();
        }
        mysqli_select_db($this->link, $this->database) or $this->err();;
...
$data = $db->getRows('select * from t_user');
//print_r($data);
//判断两个对象是否相等
if($db === $db2){
    echo 'true';
